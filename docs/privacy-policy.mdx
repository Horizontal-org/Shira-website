...
- Information about the quizzes created and managed by Admins, including the quiz name, status (published or unpublished), visibility (, public or private), and the list of quiz questions (including question name, preferences, content, and explanations).
...
- When you choose to submit a quiz or a question to the template library, as described [below](#library).

#### Template library {/* #library */}
Submitting a quiz or a question as a template to the [template library](/templates) is voluntary and always initiated by you. When you submit a template:
- The content you submit becomes publicly available to other Shira users. This includes the name of the template, its description, tags, languages, and the questions it contains (including their content and explanations). Do not include personal data or confidential information in a template.
- The template is published together with the name of the organization that contributed it, so that other users can see who the author is.
- A member of the Shira team reviews each submission before it is published, as described in our [terms and conditions](/terms-and-conditions).
- Approved templates cannot be edited. You can request the removal of a template by [contacting us](/contact). We may retain copies where reasonably necessary for legal, security, backup, or operational purposes.

Templates that you use from the library are copied into your Space. Using a template does not share any of your data back to the library.
...
- The number of quizzes started at https://quiz.shira.app, including both ready-made quizzes and custom quizzes created by Space admins
- The number of quizzes completed at https://quiz.shira.app, including both ready-made quizzes and custom quizzes created by Space admins
...
When you make a purchase through Shira, we use Stripe to process your payment.
