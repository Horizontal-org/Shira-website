---
id: self-hosting
slug: /self-hosting
sidebar_position: 3
description: Learn how to self-host your own Shira instance.

---
# Self-Hosting

Shira is [open-source](https://github.com/Horizontal-org/shira) and it's distributed under an MIT License. We maintain a cloud instance of Shira for our partners, but anyone can self-host their own instance.

Please [contact us](/contact) if you are interested in self-hosting Shira. However, experienced self-hosters may be able to do it anyway: [https://github.com/Horizontal-org/shira](https://github.com/Horizontal-org/shira). We will soon be publishing self-hosting guidance. 