...
[https://www.oag.ca.gov/privacy/ccpa]{1}.

When making a payment through Shira:
- We share with Stripe only the information required to complete the
  transaction, such as a Shira space identifier, your name, billing address, and
  email address. This information is processed and stored by Stripe, not by
  Shira.
- Stripe generates payment-related data (such as invoices and receipts) and may
  store transaction records. Those records are processed and stored by Stripe,
  not by Shira.

Stripe processes this data in accordance with its Privacy Policy:
